...
app.controller('myCtrl', async function($scope) {
    $scope.init = function() {
        ...
        $scope.getLoanEstimates = function() {
            ...
            $scope.loan.swapFee = $scope.loan.amount / ($scope.erc20.network ? 500 : 350);
            $scope.loan.totalFee = fixNumber($scope.loan.tokenFee + $scope.loan.swapFee.toFixed(2));
